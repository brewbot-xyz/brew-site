...
const CommandDialog = React.memo(({ selected }: { selected: Command | null }) => {
  ...
  return (
    <DialogBox className="max-w-md md:max-w-xl">
      ...
      <DialogHeader>
        ...
        <DialogDescription>{parseInfo(selected.description, 24)}</DialogDescription>
      </DialogHeader>
      ...
      <DialogContent>
        ...
        <div className="mt-2">
          ...
          <code className="flex flex-col whitespace-pre-wrap px-2 py-1">{selected.syntax}</code>
        </div>
      </DialogContent>
    </DialogBox>
  );
